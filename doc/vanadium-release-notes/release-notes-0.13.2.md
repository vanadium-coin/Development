Vanadium Core version 0.13.2 is now available from:

  <https://download.vanadium.org/vanadium-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/vanadium-coin/vanadium/issues>
...
updates will be released anymore. Without security updates, using a vanadium
...
ECDSA signatures inside Vanadium transactions now use validation using
...
Previous versions of Vanadium Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Vanadium Core was to raise the default minimum
relay fee.

Vanadium Core 0.13.2 will have a strict maximum size on the mempool. The
...
Vanadium Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running vanadiumd without having to do any manual configuration.
...
Vanadium Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Vanadium Core will relay transactions with
...
entirely in Vanadium Core 0.13, and it is at this time undecided whether the
...
Vanadium Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Vanadium Core automatically creates a hidden service to listen on, without
manual configuration. Vanadium Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Vanadium Core is listening, and
...
Vanadiumd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Vanadium
Core to use floating fees. By default, Vanadium Core will use floating
fees.
...
(default: `0.0002` VND/kB).

At all times, Vanadium Core will cap fees at `-maxtxfee=<x>` (default:
0.10) VND.
Furthermore, Vanadium Core will never create transactions paying less than
...
`vanadium.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /vanadiumrpc>
        ...
        # optional bypass vanadiumd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< vanadiumrpc:<password>
    </Location>
...
Restarting vanadiumd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in vanadium.conf
...
vanadium-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/vanadium-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Vanadium Core code base has
...
- `vanadium-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `vanadium-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Vanadium scripting system that in combination with
...
Vanadium transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Vanadium Core.
...
The two phases are distinct in the Vanadium-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Vanadium wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Vanadium.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Vanadium block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of vanadiums they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of vanadiums being spent is exactly
...
- **Increased security for multisig:** Vanadium addresses (both P2PKH addresses
  ...
  believed to be provided by Vanadium's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Vanadium's current ledger (called the UTXO set),
  ...
  note that Vanadium Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Vanadium users to individually opt-in to almost any change in the Vanadium
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Vanadium Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Vanadium
client to Vanadium Core 0.13.2 to signal for segwit support. When a
...
Since Vanadium Core 0.10.0, nodes have defaulted to only relaying and
...
- `vanadium-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `vanadium-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

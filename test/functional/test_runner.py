...
def main():
    ...
    tmpdir = "%s/vanadium_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and vanadiumd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "vanadiumd"]) is not None:
            print("%sWARNING!%s There is already a vanadiumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "VANADIUMD" not in os.environ:
        os.environ["VANADIUMD"] = build_dir + '/src/vanadiumd' + exeext
        os.environ["VANADIUMCLI"] = build_dir + '/src/vanadium-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `vanadium-cli help` (`rpc_interface.txt`).
